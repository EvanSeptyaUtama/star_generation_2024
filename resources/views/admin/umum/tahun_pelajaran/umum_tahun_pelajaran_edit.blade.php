...
@section('content')
    ...
    <div class="card card-borderhu border-2 rounded-3">
        ...
        <div class="card card-borderhu border-2 rounded-3">
            <div class="card-header  text-light text-center"><h3><i>{{ __('Edit Data Tahun Pelajaran') }}</i></h3></div>
                
                <div class="card-body">
                    <form action="{{ route('update_tahun_pelajaran', $data_tahun_pelajaran) }}" method="post" enctype="multipart/form-data">
                        @method('patch')
                        @csrf
                        <div class="form-group">
                            <label>Tahun Ajaran</label>
                            <input type="text" name="tahun_ajaran" class="form-control"
                                value="{{ $data_tahun_pelajaran->tahun_ajaran }}">
                        </div>
                        <div class="form-group">
                            <label>Semester</label>
                            <input type="text" name="semester"class="form-control"
                                value="{{ $data_tahun_pelajaran->semester }}">
                        </div>

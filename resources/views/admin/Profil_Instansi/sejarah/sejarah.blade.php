                            ...
                            <tbody>
                                @foreach ($data_sejarah as $data)
                                    <tr>
                                        ...
                                        <td align="justify">{{ $data->penjelasan_lengkap }}</td>
                                        <td>
                                            <img src="{{ url('storage/Sejarah/' . $data->gambar_sejarah) }}" class="rounded"
                                                style="width: 100px">
                                        </td>
                                        <td class="text-center">
                                            <div>
                                                <form action="{{ route('tampil_sejarah', $data) }}" method="get">
                                                    <button type="submit" class="btn btn-sm btn-secondary mt-2">Lihat
                                                    </button>
                                                </form>
                                                @if (Auth::user()->is_admin == true)
                                                    <form
                                                        onsubmit="return confirm('Apakah anda yakin edit data sejarah ?');"
                                                        action="{{ route('edit_sejarah', $data) }}" method="get">
                                                        <button type="submit" class="btn btn-sm btn-info mt-2">
                                                            Edit
                                                        </button>
                                                    </form>
                                                    <form
                                                        onsubmit="return confirm('Apakah anda yakin hapus data sejarah ?');"
                                                        action="{{ route('hapus_sejarah', $data->id) }}" method="post">
                                                        @csrf
                                                        @method('delete')
